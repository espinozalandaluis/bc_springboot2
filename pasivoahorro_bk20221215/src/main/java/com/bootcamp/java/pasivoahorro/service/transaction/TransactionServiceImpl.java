...
*/
    ...
    @Override
    public Mono<TransactionDTO> registerTrxEntradaExterna(TransactionDTO transactionDTO, ProductClientDTO productClientDTO) {

        return transactionRepository.save(transactionConverter.DTOtoEntity(transactionDTO))
                .flatMap(trx -> {
                    productClientDTO.setBalance(CalculateBalance(productClientDTO.getBalance(),
                            transactionDTO.getMont(),
                            Constantes.TipoTrxTransferenciaEntrada,0.0));
                    return productClientRepository.save(productClientConvert.DTOToEntity(productClientDTO))
                            .flatMap(prdcli -> {
